#!/bin/bash
# Genera COMPANION_M4.pdf da COMPANION_M4.md con le figure ancorate al testo (niente float).
set -e
cd "$(dirname "$0")"
...
python3 -c "
s=open('COMPANION_M4.md',encoding='utf-8').read()
for a,b in [('→','->'),('≡','='),('⌈','ceil('),('⌉',')'),('−','-'),('·','*'),('∝','proporzionale a'),('≫','>>'),('≪','<<')]:
    s=s.replace(a,b)
open('_companion_tmp.md','w',encoding='utf-8').write(s)
"
pandoc _companion_tmp.md -o COMPANION_M4.pdf --pdf-engine=xelatex \
  -H pandoc_header.tex \
