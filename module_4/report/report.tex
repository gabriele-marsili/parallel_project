...
\{32,64,128\} ranks on the cluster. The sequential binary now
accepts \texttt{-skew} and \texttt{-hot}, so the same triple
comparison covers the skewed case as well. All 36 local cases and
all 18 cluster cases passed; the validation logs are stored in
...
dedicated compute node. Two reference times are used, one per
workload: $t_{\text{seq}}^{\text{unif}}=\SI{4.52}{s}$ on uniform
input and $t_{\text{seq}}^{\text{skew}}=\SI{2.46}{s}$ on the
skewed input ($\rho=0.9$, four hot keys), both for
$N_R=\SI{50}{M}$, $N_S=\SI{100}{M}$. The skewed baseline is
substantially lower because four hot keys collapse the join
output to about $36$~M matches versus $200$~M for uniform,
shrinking the join phase accordingly.
...
Under the skewed dataset, measured against the dedicated skewed
baseline of \SI{2.46}{s}, the curves separate further. Pure MPI
degrades monotonically from $2.4\times$ at one node to $1.5\times$
...
slowest ranks. The hybrid version still grows, from $1.6\times$
at one node to $2.8\times$ at eight; with at most eight
participants the fan-out stays small and the work per rank is
large enough that load imbalance dominates rather than
communication latency. The smaller absolute speedups compared to
the uniform case are partly arithmetic: the skewed baseline is
$1.8\times$ faster than the uniform one to begin with, so the
distributed pipeline has less serial work to amortise.
