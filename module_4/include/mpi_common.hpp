...
// phase timing for the distributed pipeline (extends M3's PhaseTiming with the four communication / redistribution intervals)
...
// Record is { uint64_t key }, so it maps to MPI_UINT64_T on the wire
inline MPI_Datatype mpi_record_type() { return MPI_UINT64_T; }

/* splitmix64 has exact skip-ahead: each call advances the state by GOLDEN
=> seeding with (base_seed + GOLDEN * k) reproduces the k-th key the sequential generator would have emitted */
constexpr std::uint64_t SPLITMIX_GOLDEN = 0x9e3779b97f4a7c15ULL;

// local slice [offset, offset+n_local) of the uniform relation, byte-for-byte
...
/* local slice [offset, offset + n_local) of the skewed generator
The initial hot-key selection does a variable number of PRNG draws (rejection sampling),
so the per-record skip-ahead offset is unknown and each rank regenerates the
full relation and slices it. 
Generation is outside the measured region, so the extra memory is the only real cost */
...
// balanced 1D block partition of N items over P parts, returning [first, last)
