...
\begin{enumerate}
  \item \textbf{SIMD-native}: the core operation is a 32-bit multiply, which maps to \texttt{vpmulld}---a native AVX2 instruction processing 8~elements at once. A~64-bit hash would need $3\times$~\texttt{vpmuludq} decomposition, since AVX2 lacks \texttt{mullo\_epi64} (L7\&8 slide~8: \emph{``not all intrinsics map one-to-one to a single instruction''}).
...
Keys and partition IDs are stored in separate contiguous arrays (\texttt{uint64\_t[]} and \texttt{uint32\_t[]}), accessed with unit stride. This SoA-like layout is optimal for vectorization (L7\&8 slide~30, 39). Memory is aligned to 32~bytes via \texttt{posix\_memalign} for efficient AVX2 loads.
...
\textbf{Vectorizability conditions} (L7\&8 slide~34): all are satisfied---fixed trip count, no inter-iteration dependencies, no function calls, \texttt{\_\_restrict\_\_} for aliasing, unit stride access, and a SIMD-native operation.
...
Using the roofline model (L5\&6 slide~49): $P = \min(R_\text{peak},\, I \times B_\text{peak})$, with hardware parameters measured on node09:
\begin{itemize}
...
\subsection{SIMD Metrics (L10)}

Applying the L10 performance metrics to SIMD parallelism, where $p\!=\!8$ (AVX2 256-bit, 8 uint32 lanes):
...
The efficiency of $\sim$16--18\% is characteristic of memory-bound kernels: SIMD lanes spend most of their time stalled waiting for data. Interpreting via Amdahl's law (L10 slide~25), the effective serial fraction is $f \approx 64\%$, which corresponds to the memory transfer time that cannot be parallelized by SIMD. Even with $p\!\to\!\infty$ (wider SIMD or AVX-512), the theoretical maximum speedup would be only $1/f \approx 1.56\times$.

\subsection{Sweeps}

\textbf{P-sweep} (Figure~\ref{fig:speedup}): throughput and speedup are independent of $P$ (all curves flat), confirming that changing $P$ only modifies the shift constant---memory traffic and compute are unchanged.

\textbf{Key-space sweep}: throughput is invariant to the duplicate rate, as expected for a branchless hash function with constant cost per key.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{../results/plots/03_speedup_vs_N.png}
\caption{Speedup vs.\ input size ($P\!=\!256$). Stable at 1.3--1.5$\times$.}
\label{fig:speedup}
\end{figure}

\subsection{Measurement Stability}

Coefficient of variation $<5\%$ for all configurations, ensured by exclusive SLURM allocation (L4) and 11~repetitions with median reporting.
