...
All experiments were run on a node of the \texttt{spmcluster}: Intel Xeon E5-2640 v2 @ 2.00\,GHz, 2 sockets $\times$ 8 physical cores (16 physical, 32 with Hyper-Threading), NUMA architecture. Default parameters: \texttt{seed=42}, $NS=2\times NR$, \texttt{max\_key=1M}, \texttt{P=128}; best of 5 repetitions, no CPU pinning (run-to-run variation $\leq 3\%$). Strong and weak scaling fix $P=128$ to isolate the effect of thread count from partition count; the partition sensitivity analysis (\S\ref{sec:results}) independently characterises the effect of $P$ and identifies $P=512$ as the throughput-optimal value. Thread counts $p\in\{1,2,4,8,10,14,20,24,32\}$ cover the physical-core regime ($p\leq 16$) and the Hyper-Threading regime ($p>16$); the gap between $p=14$ and $p=20$ means the exact onset of HT-regime effects is not pinpointed, since the HT boundary falls at $p=16$ for 16 physical cores.
...
\end{tabular}
\end{center}
{\footnotesize Shaded rows: HT regime ($p>16$). $S(1)>1.0$: the sequential binary uses a \texttt{PartitionedRelation} struct with vector indirections, while the parallel binary uses flat arrays with direct index arithmetic; the different layouts produce slightly different cache-access patterns. The dip at $p=20$ relative to $p=16$ is a NUMA effect: threads crossing onto the second socket incur higher remote-memory latency (see \S\ref{sec:discussion}).}
...
With one counter increment per 8-byte input record, this phase is firmly memory-bandwidth-bound. Histograms account for only $\sim$7\% of sequential time, so their poor scalability affects the overall bound by $< 5\%$ (Amdahl).
...
A partitioned hash join was parallelised in C++ using a persistent team of $k$ threads coordinated by \texttt{std::barrier} (C++20), reused across all five pipeline phases to avoid per-phase spawn/join overhead. Thread-local histograms, pre-computed lock-free scatter offsets, and cyclic partition assignment eliminate all inter-thread contention in the three compute-intensive phases; the prefix sum and result accumulation remain sequential, as their $O(P)$ and $O(k)$ costs are negligible.

On the 32-logical-core Intel Xeon E5-2640\,v2 node, the implementation achieves speedups of $8.64\times$ (NR=10M) and $9.64\times$ (NR=20M) at $p=32$ with $P=128$, rising to $11.4\times$ and $11.5\times$ at the optimal $P=512$. The Amdahl-fitted serial fraction is $f\approx0.078$ ($S_\infty\approx12.9\times$) at $P=128$ and $f\approx0.058$ at $P=512$. The practical sweet spot is $p=12$--$16$ (physical cores only), where efficiency exceeds 47\%; beyond $p=16$, the NUMA crossover causes a transient dip at $p=20$ before Hyper-Threading adds moderate gains to the scatter phase.

Scatter dominates sequential time ($\approx52\%$) and is the primary weak-scaling bottleneck: its random-write pattern saturates shared DRAM bandwidth as the problem size grows with thread count. NUMA-interleaved buffer allocation (\texttt{numactl --interleave}) is the most actionable optimisation to reduce cross-socket memory penalties at $p>16$. The throughput-optimal partition count is $P=512$--$1024$, where per-partition \texttt{FlatCountMap} tables fit within the shared 20\,MB L3. All tested configurations pass correctness verification against three independent hash aggregates (\texttt{join\_count}, \texttt{checksum1}, \texttt{checksum2}).
