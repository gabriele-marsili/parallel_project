...
\]
where $k_{\max}$ is the user-requested count and $T_{\min} = 8\,192$ records is the minimum per-thread work threshold. Below this threshold, the cost of barrier synchronisation and cache-line traffic between threads exceeds the parallelism gain: with $N < k \cdot T_{\min}$, each thread's scan completes in less time than the $O(k \cdot P)$ histogram-merge step executed by the barrier delegate, so synchronisation overhead dominates useful work. For $NR = 10^7$ and $k_{\max} = 20$ the formula gives $k = \min(20, 1220) = 20$; for $NR = 10^4$ it gives $k = \min(20, 1) = 1$, falling back to a single-threaded execution with no synchronisation overhead.
...
\end{tabular}
\end{center}
{\footnotesize Individual phase times from the pipeline's internal timer; $T_\text{seq}$ (769\,ms) is their sum at $p=1$. Output buffers (\texttt{out\_R}/\texttt{out\_S}) are pre-allocated before timing begins; end-to-end speedup uses this sum as baseline. Note: the phase breakdown baseline (769\,ms) differs slightly from the strong scaling outer-timer baseline (766\,ms), which is measured by the dedicated scaling benchmark. The $p=1$ phase breakdown was measured twice (769\,ms and 753\,ms); the first (higher) value is used as baseline, giving a conservative speedup estimate within the stated $\leq3\%$ run-to-run variance.}
