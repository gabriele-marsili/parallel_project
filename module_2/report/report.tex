...
\begin{figure*}[t]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    \caption{Efficiency $E(p) = S(p)/p$, declining from 72\% at $p=2$ to 26\% at $p=32$. Shaded rows: HT threads ($p > 16$).}
...
The $p=1$ parallel binary takes $\approx 78$\,ms on NR=1M with negligible overhead from the barrier infrastructure. WSE $= T(1)/T(p)$ decreases monotonically: 0.74 at $p=2$, 0.54 at $p=10$, 0.33 at $p=32$.
...
\textbf{Histogram (2.4--3.2$\times$ at $p=20$--$32$).}
...
\textbf{Join Local (peaks at $8.1\times$ for $p=14$, $7.8\times$ at $p=32$).}
With $P=128$ and NR=10M, each partition holds $\approx 78$K records; the \texttt{FlatCountMap} occupies $\approx 4$\,MB (next-power-of-two above $2\times 78$K: 262K slots $\times$ 16\,B), well above L2 (256\,KB) but fitting in the shared 20\,MB L3. Cyclic assignment provides near-zero scheduling overhead for the uniform Fibonacci hash. The phase scales near-linearly to $p=14$ (8.1$\times$); at $p>16$ HT threads share execution ports, causing the regression described above.
...
Sequential time is essentially constant for \texttt{max\_key} $\leq 1$M ($\approx 700$--$770$\,ms): with few distinct keys the per-partition \texttt{FlatCountMap} has few occupied slots and most probes terminate immediately. At \texttt{max\_key}=10M (near-unique keys) the hash table is fully populated, increasing sequential time by $+31\%$ ($\approx 700$\,ms $\to$ 915\,ms) due to increased L3 pressure during the join phase. The parallel speedup remains stable across all duplicate densities ($\approx 6.4$--$7.5\times$ at $p=20$), confirming that partition independence is unaffected by key distribution.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{../results/plots/duplicate_density.pdf}
  \caption{Impact of duplicate density ($p=20$, NR=10M). Left: absolute execution time (seq and par). Right: speedup vs.\ \texttt{max\_key}. Both seq and par time grow with \texttt{max\_key} due to hash-table pressure; speedup remains stable ($6.4$--$7.5\times$).}
  \label{fig:dup}
\end{figure}
