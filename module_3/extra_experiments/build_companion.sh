#!/bin/bash
# Genera COMPANION_M3.pdf da COMPANION_M3.md con le figure ancorate al testo (niente float).
set -e
cd "$(dirname "$0")"
...
python3 -c "
s=open('COMPANION_M3.md',encoding='utf-8').read()
for a,b in [('→','->'),('≡','='),('⌈','ceil('),('⌉',')'),('−','-'),('·','*'),('∝','proporzionale a'),('≫','>>'),('≪','<<')]:
    s=s.replace(a,b)
open('_companion_tmp.md','w',encoding='utf-8').write(s)
"
pandoc _companion_tmp.md -o COMPANION_M3.pdf --pdf-engine=xelatex \
  -H pandoc_header.tex \
